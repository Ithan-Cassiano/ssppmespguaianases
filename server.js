...
async function handleApi(request, response) {
  ...
  if (request.method === 'GET' && url.pathname === '/api/admin/data') { if (!user) { sendJson(response, 401, { error: 'Faça login.' }); return; } const data = {}; [...modules].forEach(key => { data[key] = database[key] || []; }); data.usuarios = database.users.map(safeUser); data.config = database.config || {}; sendJson(response, 200, data); return; }
  if (request.method === 'GET' && module === 'denuncias' && pathParts.length === 3) { const item = database.denuncias.find(entry => entry.id === pathParts[2]); if (!item) { sendJson(response, 404, { error: 'Protocolo não encontrado.' }); return; } sendJson(response, 200, { id: item.id, status: item.status, createdAt: item.createdAt }); return; }
  if (request.method === 'GET' && module === 'solicitacoes' && pathParts.length === 3) { const item = database.solicitacoes.find(entry => entry.id === pathParts[2]); if (!item) { sendJson(response, 404, { error: 'Protocolo não encontrado.' }); return; } sendJson(response, 200, { id: item.id, tipo: item.tipo, status: item.status, createdAt: item.createdAt }); return; }
  ...
  if (request.method === 'POST' && url.pathname === '/api/denuncias') { const body = await readBody(request); if (!body.subject || !body.description) { sendJson(response, 400, { error: 'Assunto e descrição são obrigatórios.' }); return; } const item = { id: crypto.randomUUID(), subject: String(body.subject).trim(), description: String(body.description).trim(), contact: String(body.contact || '').trim(), alvo: String(body.alvo || '').trim(), local: String(body.local || '').trim(), anonima: Boolean(body.anonima), anexos: Array.isArray(body.anexos) ? body.anexos.filter(link => typeof link === 'string').map(link => link.trim()).filter(Boolean).slice(0, 5) : [], status: 'Recebida', createdAt: new Date().toISOString() }; database.denuncias.unshift(item); writeDatabase(database); notifyWebhook(item.anonima ? 'Nova comunicação sigilosa' : 'Nova denúncia formal', [['Protocolo', item.id], ['Assunto', item.subject], ['Alvo/local', item.alvo], ['Contato', item.anonima ? 'anônimo' : item.contact], ['Descrição', item.description]]); sendJson(response, 201, { id: item.id, status: item.status }); return; }
  ...
  if (request.method === 'DELETE' && modules.includes(module) && pathParts[2]) { if (!can(user, module)) { sendJson(response, 403, { error: 'Sem permissão para este módulo.' }); return; } database[collectionName(module)] = (database[collectionName(module)] || []).filter(item => item.id !== pathParts[2]); writeDatabase(database); sendJson(response, 200, { ok: true }); return; }
  if (request.method === 'PATCH' && modules.includes(module) && pathParts[2]) { if (!can(user, module)) { sendJson(response, 403, { error: 'Sem permissão para este módulo.' }); return; } const collection = database[collectionName(module)] || []; const item = collection.find(entry => entry.id === pathParts[2]); if (!item) { sendJson(response, 404, { error: 'Registro não encontrado.' }); return; } const body = await readBody(request); const { id, createdAt, ...rest } = body; Object.assign(item, rest, { updatedAt: new Date().toISOString() }); writeDatabase(database); sendJson(response, 200, item); return; }
