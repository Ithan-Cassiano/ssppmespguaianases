...
async function handleApi(request, response) {
  ...
  if (request.method === 'GET' && module === 'protocolo' && pathParts.length === 3) {
    ...
    const edital = (database.editais || []).find(item => String(item.protocolo || '').toLowerCase() === code.toLowerCase());
    if (edital) { sendJson(response, 200, { type: 'edital', id: edital.id, title: edital.title, status: edital.status, protocolo: edital.protocolo, aprovados: edital.aprovados || [], date: edital.date || '' }); return; }
